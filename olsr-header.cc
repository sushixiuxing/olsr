...
#define NODEID_SIZE 1
#define OLSR_MSG_HEADER_SIZE 7
#define OLSR_PKT_HEADER_SIZE 2
...
PacketHeader::Serialize (Buffer::Iterator start) const
{
  ...
  //修改
    i.WriteU8((uint8_t)m_packetLength);
    i.WriteU8 ((uint8_t)m_packetSequenceNumber);
}
...
PacketHeader::Deserialize (Buffer::Iterator start)
{
 ...
 // m_packetSequenceNumber = i.ReadNtohU16 ();
  //修改
  m_packetLength  = (uint16_t)i.ReadU8 ();
  m_packetSequenceNumber = (uint16_t)i.ReadU8 ();
...
MessageHeader::Serialize (Buffer::Iterator start) const
{
 ...
 // i.WriteHtonU32 (m_originatorAddress.Get ());
  //修改
  i.WriteU8 ((uint8_t)GetSerializedSize ());
  i.WriteU8 ((uint8_t)(m_originatorAddress.Get ()));
  i.WriteU8 (m_timeToLive);
  i.WriteU8 (m_hopCount);
 //.WriteHtonU16 (m_messageSequenceNumber);
  //修改
  i.WriteU8 (m_messageSequenceNumber);
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  m_vTime  = i.ReadU8 ();
 //_messageSize  = i.ReadNtohU16 ();
  m_messageSize  = i.ReadU8 ();
   uint8_t buf1[]={10,1,1,i.ReadU8 ()};
   Ipv4Address ip2;
   m_originatorAddress =ip2.Deserialize (buf1);
   m_timeToLive  = i.ReadU8 ();
   m_hopCount  = i.ReadU8 ();
 //_messageSequenceNumber = i.ReadNtohU16 ();
  m_messageSequenceNumber =(uint16_t)i.ReadU8();
...
MessageHeader::Hello::GetSerializedSize (void) const
{
  ...
  for (std::vector<LinkMessage>::const_iterator iter = this->linkMessages.begin ();
       iter != this->linkMessages.end (); iter++)
    {
      ...
      size += 2;
      size += NODEID_SIZE * lm.neighborInterfaceAddresses.size ();
    }
  return size;
}
...
MessageHeader::Hello::Serialize (Buffer::Iterator start) const
{
 ...
 // i.WriteU16 (0); // Reserved
  ...
  for (std::vector<LinkMessage>::const_iterator iter = this->linkMessages.begin ();
   ...
   //NS_LOG_UNCOND ((uint16_t)lm.linkCode);
   //   i.WriteU8 (0); // Reserved
      ...
      i.WriteU8 (2 + lm.neighborInterfaceAddresses.size () * NODEID_SIZE);
   // NS_LOG_UNCOND ((uint16_t)(2 + lm.neighborInterfaceAddresses.size () * NODEID_SIZE));
      for (std::vector<Ipv4Address>::const_iterator neigh_iter = lm.neighborInterfaceAddresses.begin ();
           neigh_iter != lm.neighborInterfaceAddresses.end (); neigh_iter++)
        {
         // i.WriteHtonU32 (neigh_iter->Get ());
          i.WriteU8((uint8_t)neigh_iter->Get ());
         // NS_LOG_UNCOND (neigh_iter->Get ());
...
MessageHeader::Hello::Deserialize (Buffer::Iterator start, uint32_t messageSize)
{
  ...
  NS_ASSERT (messageSize >= 2);

  this->linkMessages.clear ();

  uint16_t helloSizeLeft =(uint16_t) messageSize;
  ...
  while (helloSizeLeft)
    {
      LinkMessage lm;
      NS_ASSERT (helloSizeLeft >= 2);
      lm.linkCode = i.ReadU8 ();
    //  i.ReadU8 (); // Reserved
     // uint16_t lmSize = i.ReadNtohU16 ();
      uint16_t lmSize=(uint16_t)i.ReadU8 ();
      ...
      //  }
       Ipv4Address ip1;
        NS_ASSERT ((lmSize - 2) % NODEID_SIZE == 0);
      for (int n = (lmSize - 2) / NODEID_SIZE; n; --n)
        {
          uint8_t buf[]={10,1,1,i.ReadU8 ()};
         // NS_LOG_UNCOND ((uint16_t)buf[4]);

        lm.neighborInterfaceAddresses.push_back (ip1.Deserialize (buf));
        //NS_LOG_UNCOND (ip1.Deserialize (buf));
        }
...
MessageHeader::Tc::GetSerializedSize (void) const
{
  ...
  return 1 + this->neighborAddresses.size () * NODEID_SIZE;
   NS_LOG_UNCOND (this->neighborAddresses.size ()); //5
...
MessageHeader::Tc::Deserialize (Buffer::Iterator start, uint32_t messageSize)
{
  ...
  //i.ReadNtohU16 (); // Reserved
  this->ansn = (uint16_t)i.ReadU8 ();
  NS_ASSERT ((messageSize - 1) % NODEID_SIZE == 0);
  //int numAddresses = (messageSize - 4) / IPV4_ADDRESS_SIZE;
  int numAddresses = (messageSize - 1) / NODEID_SIZE;
  //NS_LOG_UNCOND (numAddresses ); //5
  this->neighborAddresses.clear ();
  for (int n = 0; n < numAddresses; ++n)
    {  uint8_t buf[]={10,1,1,i.ReadU8 ()};
      Ipv4Address ip;
       this->neighborAddresses.push_back (ip.Deserialize (buf));
      //NS_LOG_UNCOND ("tc"); //5
      // NS_LOG_UNCOND (ip.Deserialize (buf)); //5
///NS_LOG_UNCOND (neighborAddresses.size());
    }
