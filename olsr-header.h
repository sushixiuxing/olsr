...
public:
  ...
  // 修改 增加
  uint32_t ReturnNodeId (uint8_t id)
  { 
      uint8_t buf[4]={10,1,1,id};
      Ipv4Address ip;
      ip.Deserialize (buf);
      return ip.Get();
  }
  uint8_t GetNodeId (uint32_t ipv4Address) const
  {
    uint8_t m_node = (uint8_t) ipv4Address;
    return m_node;
  }
   //
private:
...
public:
  ...
  virtual void Serialize (Buffer::Iterator start) const;
  virtual uint32_t Deserialize (Buffer::Iterator start);
  //virtual uint32_t ReturnNodeId (uint8_t id);
  /**
   * \ingroup olsr
   * MID Message Format
   *
